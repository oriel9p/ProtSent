...
# The partial run was deleted: a quarantined directory that still generated tables was one bad
# glance away from being quoted as a result. Nothing silently substitutes for full coverage now --
# if the benchmarks dir is not full, say so and stop rather than reporting a subset as if it were.
D = Path(os.environ["PG_DIR"]) if os.environ.get("PG_DIR") else _B
if not os.environ.get("PG_DIR") and not _proteingym_is_full(_B / "proteingym_maxsim.csv"):
    sys.exit(f"{_B/'proteingym_maxsim.csv'} is not a full-coverage run and the partial fallback was "
             f"deleted. Rerun ProteinGym, or point PG_DIR at a directory you have checked.")
